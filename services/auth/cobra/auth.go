...
func Auth(ctx context.Context, config *viper.Viper) *cobra.Command {
	c := &cobra.Command{
		Use: "auth",
	}
	...
	c.AddCommand(createToken(ctx, config))
	return c
}

func createToken(ctx context.Context, config *viper.Viper) *cobra.Command {
	c := &cobra.Command{
		Use:     "create-token",
		Aliases: []string{"ct"},
